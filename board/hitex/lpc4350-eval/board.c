	...
	lpc18xx_get_boot_source(void)
{
	...
	rv = (LPC18XX_GPIO_B(1, 1) << 0) |
	     (LPC18XX_GPIO_B(1, 2) << 1) |
	     (LPC18XX_GPIO_B(2, 8) << 2) |
	     (LPC18XX_GPIO_B(2, 9) << 3);
...
static const struct lpc18xx_pin_config
	__attribute__((section(".lpc18xx_image_top_data")))
	hitex_lpc4350_iomux_boot_spiflash[] = {
	/*
...
static void __attribute__((section(".lpc18xx_image_top_text")))
	spiflash_bootstrap_iomux_init(void)
{
	lpc18xx_pin_config_table(
		hitex_lpc4350_iomux_boot_spiflash,
		ARRAY_SIZE(hitex_lpc4350_iomux_boot_spiflash));
}
...
void __attribute__((section(".lpc18xx_image_top_text")))
	lpc18xx_bootstrap_from_spifi(void)
{

		/*
		 * Configure remaining pins required for SPIFI
		 */
		spiflash_bootstrap_iomux_init();
