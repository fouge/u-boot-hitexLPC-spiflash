...
/* check if target device is software protected */
  ...
  spifi_lpc_cmd(SPI_RDSR_CMD, (ADDRESS)ADDRESS_NOT_USED, 0, (*target));
...
/* check if target device is software protected */
  ...
  spifi_lpc_cmd(SPI_WREN_CMD, 0, 0, 0);
 debug("Command reg : 0x%x", *((unsigned int *)0x40003004));
  return(status);
}
...
/* check if target device is software protected */
  ...
  spifi_lpc_cmd(SPI_WRDI_CMD, (ADDRESS)ADDRESS_NOT_USED,0, BUFFER_NOT_USED);
  return(status);
}
...
/* check if target device is software protected */
  ...
  spifi_lpc_cmd(SPI_WRSR_CMD,(ADDRESS)ADDRESS_NOT_USED, 0, (*data_buf));
...
* If config value is NULL, the config value will not be sent.
* RETURNS: SLLD_OK, SLLD_E_DEVICE_SOFTWARE_PROTECTED or SLLD_E_HAL_ERROR
*
*/
...
/* check if target device is software protected */
  ...
  spifi_lpc_cmd(SPI_RCR_CMD, ADDRESS_NOT_USED, 0, (*target));
  return(status);
}
#endif /* REMOVE_RCRCmd */
...
)
{
  ...
  debug("Write status register command..");
...
)
{
  ...
  debug("Reading current status register..");
...
/******************************************************************************
*
* slld_RDRS2Cmd - Read from Status Register-3
*
* This function issues the RDSR2 command to SPI Flash and reads from status register.
*
* RETURNS: SLLD_OK, SLLD_E_DEVICE_SOFTWARE_PROTECTED or SLLD_E_HAL_ERROR
